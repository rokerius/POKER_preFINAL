...
class Game(object):
    def __init__(self):
        ...
        while True:
            ...
            if 1 < self.number_of_players < 11 and self.starting_chips > 0 and \
                ...
                print("(!) Слишком мало или много игроков")
    ...
    def find_winners(self):  # Ищем победителя
        ...
        if self.fold_out:
            ...
            max_score = max(list_of_stakes)
            for player in self.list_of_players_not_out:
                if player.score == max_score and not player.fold:
                    player.win = True
                    self.winners.append(player)
            self.winners = list(set(self.winners))
    ...
    def end_round(self):
        print(print("\n --------GLOBAL winners--------"))
        self.list_of_players_not_out = self.list_of_players_not_out
        for player in self.list_of_players_not_out:
            if player.chips <= 0:
        ...
        if len(set(self.list_of_players_not_out)) == 1:
            ...
            print(f"Остался только: {self.list_of_players_not_out[0]} и него фишек:"
                  f" {self.list_of_players_not_out[0].chips}!")
            quit()
    ...
    def answer(self, player):  # Получаем ответ у игрока
        ...
        if player.stake_gap > 0:
            ...
            if player.stake_gap < player.chips:
                ...
                self.possible_responses.append("call + raise")
                self.possible_responses.append("call + all_in")
        if player.stake_gap == 0:
        ...
        while True:
            ...
            if response == "call + raise":
                ...
                while True:
                    ...
                    player.stake += bet  # Вклад
                    self.pot += bet  # Банк
                    player.chips -= bet  # Фишки игрока
                    self.highest_stake = player.stake
                    self.count_smth = 0
                    return True
            if response == "call + all_in" or response == "all_in":
                ...
                self.count_smth = 0
                return True
            if response == "all_in_partial" or response == "all_in_exact":
